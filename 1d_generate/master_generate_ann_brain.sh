...
WEIGHTS=("IMAGENET1K_SWAG_E2E_V1") #("ResNet50_Weights.IMAGENET1K_V2")  IMAGENET1K_SWAG_E2E_V1
MODELS=("vit_b_16") #("resnet50") vit_b_16
LAYER=("encoder.layers.encoder_layer_11.mlp")
TRAINED_MODEL_FILES=(237393) #(197972) #(211379 210760) #(197894 197972) # 201512 (197894 197972)
NUM_GENERATION_STEPS=(100) # 250 400 500)
        ...
        for cond_strength in "${CONDITIONING_STRENGTH[@]}"; do
            for ode in "${ODES[@]}"; do  
                ...
                for num_steps in "${NUM_GENERATION_STEPS[@]}"; do
                    ...
                    sbatch -p gpu1 1d_generate/generate_ann_brain.sh $roi $weight $layer $model $roi_file $cond_strength $trained_model_file $ode $num_steps
                done
            done
        done
    done
